...
<div class="page-content">
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        ...
        <div class="ms-auto">
            ...
            <a href="{{URL::to('/add_stock')}}" class="btn btn-primary px-5 radius-30">
    ...
    <h6 class="mb-0 text-uppercase">All Stock</h6>
    <hr />
    <form action="" method="GET">
        ...
        <div class="row">
            ...
            <div class="col-md-3 mt-4">
                ...
                @if($filter == 1)
                <a href="{{URL::to('stock_list')}}" class="btn btn-danger btn-sm">Remove Filter</a>
                @endif
            </div>
        </div>
    </form>
    ...
    <div class="card">
        <div class="card-body">
            <div style="text-align:center;">
                ...
                <a class="toggle-vis btn btn-sm btn-primary" data-column="5">Qty</a>
                ...
                <a class="toggle-vis btn btn-sm btn-primary" data-column="8">Cost/Unit</a>
                <a class="toggle-vis btn btn-sm btn-primary" data-column="9">Amazon Fee</a>
                <a class="toggle-vis btn btn-sm btn-primary" data-column="10">Sale Price</a>
                <a class="toggle-vis btn btn-sm btn-primary" data-column="11">Unit Profit</a>
                <a class="toggle-vis btn btn-sm btn-primary" data-column="12">Profit %</a>
                <a class="toggle-vis btn btn-sm btn-primary" data-column="13">Expected Profit</a>
            </div>
                ...
                <table id="example" class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th style="width:80px">Expected Profit</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @php
                                $cartons = round($row->stock_qty / $row->p_units_in_carton , 2);
                                $total_carton += $cartons;
                                @endphp
                                {{ $cartons }}
                            </td>
                            <td>
                            ...
                            <td>
                                @php
                                $expected_profit =  round(($row->item_sale_price - ($row->cost_per_unit + $amazon_fee )) * $row->stock_qty , 2);
                    ...
                    <tfoot style="background: bisque;font-weight: bold;">
                        <tr>
                            <th colspan="5" style="text-align: right;"> <strong>Totals : </strong></th>
                            <th>{{$total_qty}}</th>
                            <th>{{$total_carton}}</th>
                            <th>{{$total_purchas_price}}</th>
                            <th>{{$total_cost_per_unit}}</th>
                            <th>{{$total_amazon_fee}}</th>
                            <th>{{$total_sale_price}}</th>
                            <th>{{$total_unit_profit}}</th>
                            <th>{{$total_profit_percentage}}</th>
                            <th>{{$total_expected_profit}}</th>
                            <th>{{$total_cost}}</th>
                        </tr>
                    </tfoot>
