...
<div style="padding: 30px; font-family: 'Exo' !important;">
    <div style="margin-top: 0px; margin-left: 4px;">
        ...
        <div style="float: right; clear: both;">
            <span style="font-size: 20px; letter-spacing: 2px; color:#9f1d1d; text-decoration: underline;"><strong>Transfer
                    Stock Voucher</strong></span><br>
            <span style="font-size: 14px; float:right;">Stock Ref #: {{ $stock[0]->stock_refrence_no }}</span><br>
            <span style="font-size: 14px;float:right;">Date: {{ date_view($stock[0]->stock_entry_date) }}</span><br>
            <span style="font-size: 14px;float:right;">Type:
                @if($stock[0]->type == 1)
                Stock Added
                @else
                Stock Transfer
    ...
    <div style="margin-top: 0px; margin-left: 4px; clear: both;">
        <div style="width: 100%;border: 1px solid #0a0a0a;margin-top: 15px;border-radius: 5px;">
            <span style="font-size: 20px; color:#9f1d1d; margin-left:15px;">
                <strong>Warehouse Info</strong></span><br>

            @php
            $warehouse = get_warehouse_details($stock[0]->wh_id_to);
            @endphp
            <span style="font-size: 14px;padding-left: 15px;"> {{ $warehouse->wh_title}}</span><br>
            @if($warehouse->wh_contactperson !="")
            <span style="font-size: 14px;padding-left: 15px;">{{ $warehouse->wh_contactperson}}
            </span><br>
            @endif
            @if($warehouse->wh_contactnumber !="")
            <span style="font-size: 14px;padding-left: 15px;">{{ $warehouse->wh_contactnumber}}</span><br>
            @endif
            <span style="font-size: 14px;padding-left: 15px;">{{ $warehouse->wh_location}} </span><br>
        </div>
    </div>
